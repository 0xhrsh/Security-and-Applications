...
def xor(a, b):
    ...
    return dec2bin(ans, len(a1))


# Initial Permutation Table
INITIAL_PRMT = [58, 50, 42, 34, 26, 18, 10, 2,
...
FINAL_PRMT = [40, 8, 48, 16, 56, 24, 64, 32,
...
KEY_PRIORITY = [57, 49, 41, 33, 25, 17, 9,
                1, 58, 50, 42, 34, 26, 18,
                10, 2, 59, 51, 43, 35, 27,
                19, 11, 3, 60, 52, 44, 36,
...
def getSBoxStr(right_expanded, rkbi):
    xor_x = xor(right_expanded, rkbi)
    sbox_str = ""
    for j in range(0, 8):
        j_ = j * 6
        row = bin2dec(xor_x[j_] + xor_x[j_ + 5])
        col = bin2dec(
            xor_x[j_ + 1] + xor_x[j_ + 2] + xor_x[j_ + 3] + xor_x[j_ + 4])
        val = SBOX[j][row][col]
...
if __name__ == "__main__":
    ...
    for i in range(0, N_ROUNDS):
        ...
        round_key = permute(left + right, KEY_COMPRESSION, 48)
